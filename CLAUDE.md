...
Current Claude API pricing (verified 2026-09-05):

- **Fable 5.1** (most capable widely released model, `claude-fable-5-1`): **$10/$50 per 1M input/output** -- same posted rate as Fable 5, 2x Opus 5's. 1M context at standard rates (1M is both default and max). 128K max output. **Cache hit $0.25 -- 0.025x base input, the ONLY model tier that breaks the universal 0.1x cache rule** (a quarter of Fable 5's $1). 5m-write $12.50, 1h-write $20. Batch $5/$25. Min cacheable prompt **512**. Because a hit is so cheap relative to a miss, a `max_tokens: 0` keep-alive on the 5-minute TTL usually beats paying the 2x 1-hour write. Adaptive thinking **always on** (`thinking: {type: "disabled"}` and `budget_tokens` both return 400; control depth with `effort`, which supports `low` through `xhigh` and `max`). **Three breaking changes vs Fable 5**: forced tool use (`tool_choice` `any`/`tool`) returns 400; thinking blocks are bound to the producing model (other models drop them, unbilled); and editing earlier turns invalidates thinking blocks ("preserved thinking" -- accounts created on/after 2026-08-31 get a 400 on edited history, so keep harnesses append-only). Adds per-message `effort` (beta `mid-conversation-output-config-2026-07-01`), turn-scoped `clear_at: "next_user_message"` system messages (beta), `thinking.display: "updates"` progress notes (beta), and content provenance. **No Fast Mode. No Priority Tier.** Requires 30-day data retention (ZDR returns `400 invalid_request_error` unless expressly authorized). Same tokenizer as Fable 5 (~30% more tokens than pre-4.7 models). GA 2026-09-04.
- **Mythos 5.1** (limited availability, Glasswing, `claude-mythos-5-1`): same specs, pricing, and **$0.25 cache-read rate** as Fable 5.1. Unlike Mythos 5 it runs access-program safeguards, so `stop_reason: "refusal"` can occur. Successor to Mythos 5. Approved Project Glasswing customers only; **not offered on Claude Platform on AWS**.
- **Fable 5** (legacy, `claude-fable-5`): $10/$50 -- same posted rate as Fable 5.1, but **cache hit $1 (0.1x) versus Fable 5.1's $0.25**, so migrating is strictly cheaper on any cached workload. 1M context, 128K max output. 5m-write $12.50, 1h-write $20. Batch $5/$25. Min cacheable prompt 512. Always-on adaptive thinking; no Fast Mode. Requires 30-day retention. Still served and selectable by id. GA 2026-06-09.
- **Mythos 5** (legacy, Glasswing, `claude-mythos-5`): same specs and pricing as Fable 5, including the $1 cache read, but **without the safety classifiers**, so `refusal` never occurs. Superseded by Mythos 5.1.
...
- **Opus 4.1** (**RETIRED 2026-08-05** on the Claude API; still served on Bedrock and Google Cloud): $15/$75. 200K context. 32K max output. Cache hit $1.50. Extended thinking only. Migrate to Opus 5.
- **Sonnet 5** (Sonnet-tier flagship, `claude-sonnet-5`): **$2/$10 -- now the permanent standard price.** The launch "introductory" rate through 2026-08-31 was made standard, and the scheduled increase to $3/$15 on 2026-09-01 **will not occur**. That makes Sonnet 5 **60% cheaper than Opus 5**, not 40%. Cache hit $0.20, 5m-write $2.50, 1h-write $4. Batch $1/$5. 1M context, 128K max output. Min cacheable prompt 1,024. Earliest retirement 2027-06-30.
...
- 1M context on Fable 5.1, Mythos 5.1, Fable 5, Mythos 5, Opus 5, Opus 4.8, Opus 4.7, Opus 4.6, Sonnet 5, and Sonnet 4.6 is at **standard rates** -- no long-context premium. Opus 4.5, Sonnet 4.5, Opus 4.1, and Haiku 4.5 are 200K-only.
- Batch API: 50% discount on both input AND output (Opus 5 batch = $2.50/$12.50). Cache writes: 1.25x base input (5-min), 2x base input (1-hour). Batch supports up to **300K output** on Opus 5/4.8/4.7/4.6 + Sonnet 5/4.6 via `output-300k-2026-03-24` beta header.
- **Cache hit = 0.1x base input on every model EXCEPT Fable 5.1 and Mythos 5.1, which read at 0.025x ($0.25/MTok).** This is new as of Fable 5.1 and it breaks the "cache hits are 90% off" shorthand the rest of this repo relies on -- on those two models a hit is **97.5% off**. Never derive a cache rate by multiplying input by 0.1; read the per-model rate. Practical consequence: on Fable 5.1 a miss is far more expensive *relative to* a hit, so prefer keeping the 5-minute entry alive with a `max_tokens: 0` re-send over paying the 2x 1-hour write.
- **Minimum cacheable prompt length** (a `cache_control` block below the floor is *silently* ignored -- no error, no `cache_creation_input_tokens`, full input price every turn): Opus 5 / Fable 5.1 / Mythos 5.1 / Fable 5 / Mythos 5 = **512**; Opus 4.8 / Opus 4.1 / Sonnet 5 / Sonnet 4.6 = **1,024**; Opus 4.7 / Haiku 3.5 = **2,048**; Opus 4.6 / Opus 4.5 / Sonnet 4.5 / Haiku 4.5 = **4,096**. Size your cached prefix against the **highest** floor you route to.
...
**Bedrock**: Opus 5 GA, model ID `anthropic.claude-opus-5` (legacy InvokeModel/Converse path: cross-region `us.anthropic.claude-opus-5`); Vertex AI `claude-opus-5`. Fable 5.1 available via Claude in Amazon Bedrock (Messages-API endpoint), model ID `anthropic.claude-fable-5-1`; the old Bedrock-specific minimum-cacheable-prompt override for Fable 5.1 was removed, so the 512-token floor now applies on every platform. Fable 5 `anthropic.claude-fable-5` (legacy InvokeModel/Converse path: cross-region `us.anthropic.claude-fable-5`). Opus 4.8 GA, model ID `anthropic.claude-opus-4-8`. Opus 4.7 = `anthropic.claude-opus-4-7`. Opus 4.6 = `anthropic.claude-opus-4-6` (Mantle) or `anthropic.claude-opus-4-6-v1` (legacy). Legacy InvokeModel/Converse path uses cross-region `us.anthropic.claude-opus-4-*` IDs for backward compat.

**Subscriptions**: Pro $20/mo or **$200/yr (~$16.67/mo, 17% off)**. Max 5x $100/mo. Max 20x $200/mo.

**Recently retired** (will fail): Opus 4.1 (`claude-opus-4-1-20250805`) retired **2026-08-05** on the Claude API -- still served on Bedrock and Google Cloud; migrate to Opus 5. Mythos Preview (`claude-mythos-preview`) retired 2026-06-30 -- migrate to Mythos 5.1. Sonnet 4 (`claude-sonnet-4-20250514`) and Opus 4 (`claude-opus-4-20250514`) retired 2026-06-15. Haiku 3 (`claude-3-haiku-20240307`) retired 2026-04-20. Haiku 3.5 (`claude-3-5-haiku-20241022`) retired 2026-02-19 (still on Bedrock + Vertex AI). Sonnet 3.7 (`claude-3-7-sonnet-20250219`) retired 2026-02-19. Opus 3 (`claude-3-opus-20240229`) retired 2026-01-05.

**Upcoming retirement**: Sonnet 4.5 (`claude-sonnet-4-5-20250929`) retires **2026-09-29** -- the next one due. Haiku 4.5 (`claude-haiku-4-5-20251001`) 2026-10-15. Opus 4.5 (`claude-opus-4-5-20251101`) 2026-11-24.
