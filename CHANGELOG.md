...
- **All 21 pricing-provenance stamps now agree at 2026-09-05.** 16 files still read 2026-07-25 while carrying correct 2026-09-05 data, and 5 (`site/src/utils/pricing.ts`, `tools/mcp-cost-server/src/index.ts`, `tools/token-estimator/estimate.py`, `tools/vscode-extension/src/costEstimator.ts`, plus the README's own cross-reference) already read 2026-09-05 -- so the drift ran both ways and `claude-rate` printed "Verified against Anthropic pricing as of: 2026-07-25" while listing Fable 5.1, a model that did not exist until 2026-09-04.
...
- `.claude/settings.json`, tracked, pinning a default model, a budget cap, and permissions, and wiring `budget-tracker.sh`, `cost-logger.sh`, and `session-summary.sh` with repo-relative paths. The shipped hooks are now exercised in the repo that ships them instead of only being documented; `hooks/README.md` points at it as a runnable reference rather than a placeholder template.
...
- `scripts/check-pricing-sync.py` -- fails on verified-date drift between files, on any model described as "retiring" on a date that has already passed, and on relative countdowns like "about 11 days out". Verified against the pre-fix tree: it reports 14 problems there, including all 6 stale retirements and the countdown, and 0 on the fixed tree.
