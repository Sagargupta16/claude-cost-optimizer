...
> Opus 4 vs Sonnet 4 vs Haiku 3.5 across five common task types. Find the best cost-to-quality ratio for each kind of work.
...
| | Recommendation |
|---------|----------------|
| **Best Value** | Haiku 3.5 — 16x cheaper than Opus, identical quality |
| **Best Quality** | Tie — all models score 5/5 |
| **Avoid** | Opus 4 — paying $0.65 for a task Haiku does for $0.04 is pure waste |
...
| | Recommendation |
|---------|----------------|
| **Best Value** | Sonnet 4 — 5x cheaper than Opus, nearly equal quality (4/5), one minor fix |
| **Best Quality** | Opus 4 — perfect output, zero iterations needed |
| **Avoid** | Haiku 3.5 for complex components — follow-up turns erode cost savings and quality suffers |

> **Rule of thumb**: For components with complex types, state logic, or multiple interaction patterns, Sonnet is the sweet spot. Use Opus when the cost of getting it wrong (rework time) exceeds the $1.36 premium.
...
| | Recommendation |
|---------|----------------|
| **Best Value** | Sonnet 4 — finds the primary issue quickly, reasonable cost, acceptable that it needs a nudge for the secondary issue |
| **Best Quality** | Opus 4 — finds both causes in a single investigation pass |
| **Avoid** | Haiku 3.5 for debugging — low accuracy means more developer time spent steering, which defeats the purpose |
...
| | Recommendation |
|---------|----------------|
| **Best Value** | Sonnet 4 — covers 90% of what Opus produces at 19% of the cost |
| **Best Quality** | Opus 4 — the plan is production-ready without modification |
| **Avoid** | Haiku 3.5 — output requires so much developer augmentation that it saves neither time nor money |
...
| Finding | Severity | Opus 4 | Sonnet 4 | Haiku 3.5 |
|---------|:--------:|:------:|:--------:|:---------:|
...
| | Recommendation |
|---------|----------------|
| **Best Value** | Sonnet 4 — catches the high-impact issues at 19% of Opus's cost |
| **Best Quality** | Opus 4 — catches subtle correctness issues that could become production bugs |
| **Avoid** | Haiku 3.5 for security/correctness review — missing critical issues makes it counterproductive |
...
| Task Type | Haiku 3.5 | Sonnet 4 | Opus 4 | Recommendation |
|-----------|:---------:|:--------:|:------:|----------------|
| Formatting / renaming | 5/5 — $0.04 | 5/5 — $0.13 | 5/5 — $0.65 | **Use Haiku** |
| Component creation | 3/5 — $0.11 | 4/5 — $0.33 | 5/5 — $1.69 | **Use Sonnet** (Opus for complex types) |
| Bug investigation | 2/5 — $0.15 | 4/5 — $0.44 | 5/5 — $1.74 | **Use Sonnet** (Opus for hard bugs) |
| Architecture planning | 2/5 — $0.10 | 4/5 — $0.36 | 5/5 — $1.90 | **Use Sonnet** (Opus for critical design) |
| Code review | 3/5 — $0.05 | 4/5 — $0.19 | 5/5 — $1.02 | **Use Sonnet** (Opus for security review) |
...
|-----------|:-----------------:|:------------------:|:-----------------:|
| Formatting / renaming | **125.0** | 38.5 | 7.7 |
| Component creation | 27.3 | **12.1** | 3.0 |
| Bug investigation | 13.3 | **9.1** | 2.9 |
| Architecture planning | 20.0 | **11.1** | 2.6 |
| Code review | 60.0 | **21.1** | 4.9 |

> Higher is better. Haiku dominates for simple tasks. Sonnet wins for everything else on a quality-per-dollar basis. Opus is the right choice only when the cost of imperfect output (rework, missed bugs, production incidents) exceeds its premium.
...
└── No
    ...
    │           ├── Yes → Use Sonnet 4 (with plan mode to reduce iterations)
    │           └── No → Use Sonnet 4 (default) or Opus 4 (for peace of mind)
```
...
|----------|:------------:|:-------:|
| Always Opus | $117.60 | Excellent |
| Always Sonnet | $24.40 | Good |
| Always Haiku | $6.88 | Poor (for complex tasks) |
| **Smart routing** (Haiku for simple, Sonnet for medium, Opus for critical) | **$18.20** | **Good to Excellent** |

> Smart model routing saves **84%** versus always-Opus while maintaining high quality where it matters.
