...
**Why this saves money**: The Grep results (potentially thousands of lines) load into the cheap subagent's context, not the expensive Opus context. The main context only receives the summarized result.
...
**Why this saves money**: Opus only processes the summaries, not the raw files. The heavy reading is done by Haiku at 1/19th the cost.
...
|----------|:-----:|:-------------:|:-----:|
| Unoptimized (all Opus, no planning, no batching) | 25 | $0.35 | $8.75 |
| Partially optimized (Sonnet default, some planning) | 18 | $0.12 | $2.16 |
| Fully optimized (right models, planning, batching, /compact) | 12 | $0.08 | $0.96 |

**Fully optimized is 89% cheaper** than the unoptimized approach, for the same end result.
